...
function getWinners(arr, getFinals) {
    return getFinals(arr).map(item => item['Home Team Goals'] > item ['Away Team Goals']?
...
function getWinnersByYear(arr, getFinals, getYears, getWinner) {
    const winners = getWinner(arr, getFinals);
...
function getAverageGoals(getFinals) {
   const aveHomeG = getFinals.reduce(function (acc, item){
       return acc + item['Home Team Goals'] + item['Away Team Goals'];
   }, 0)
   return( aveHomeG/ getFinals.length).toFixed(2);
}
